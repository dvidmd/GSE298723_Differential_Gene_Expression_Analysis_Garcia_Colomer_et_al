---
title: "HEATMAP_CODE_WT_NT_KO_NT_AND_KO_R24"
...
expr_raw <- read.delim(
  "WT_NT_KO_NT_AND_KO_R24_VST_TRANSFORMED.txt",
  header           = TRUE,
  sep              = "\t",
  check.names      = FALSE,
...
metadata_hp <- subset(metadata, Group %in% c("WT_NT", "KO_NT", "KO_R24")) |>
  select(Group)

names(metadata_hp) <- "Condition"

##############################################
# Final order and levels: WT → KO → KO + R24 #
##############################################
...
metadata_hp$Condition <- factor(
  dplyr::case_when(
    ...
    grepl("KO_R24",  rownames(metadata_hp)) ~ "KO + R24",
    TRUE ~ metadata_hp$Condition
  ),
  levels = c("WT", "KO", "KO + R24")
)
...
colnames(expr_hp) <- gsub("_KO_R24$",  "_KOR24", colnames(expr_hp))
...
save_heatmap("HEATMAP_DEG_FROM_FDR_0_05_FC_1_5_WT_NT_VS_KO_NT_IN_WT_NT_AND_KO_NT_AND_KO_R24_VST_NO_COLUMNS_NAMES_NO_ANNOTATION",
             show_colnames = FALSE, add_annotation = FALSE)

save_heatmap("HEATMAP_DEG_FROM_FDR_0_05_FC_1_5_WT_NT_VS_KO_NT_IN_WT_NT_AND_KO_NT_AND_KO_R24_VST_WITH_COLUMNS_NAMES_WITH_ANNOTATION",
             show_colnames = TRUE,  add_annotation = TRUE)

save_heatmap("HEATMAP_DEG_FROM_FDR_0_05_FC_1_5_WT_NT_VS_KO_NT_IN_WT_NT_AND_KO_NT_AND_KO_R24_VST_WITH_COLUMNS_NAMES_NO_ANNOTATION",
