---
title: "WT_NT_vs_WT_NT_CONTROL_VS_TREATMENT"
...
ddsHTSeq$condition <- relevel(ddsHTSeq$condition, ref = "WT_NT")
...
library(apeglm)
condition_KO_NT_vs_WT_NT <- lfcShrink(dds, coef="condition_KO_NT_vs_WT_NT", type="apeglm")
...
filterValues <- rownames(condition_KO_NT_vs_WT_NT)
...
colnames(annot)
colnames(condition_KO_NT_vs_WT_NT)

annot_final <- as.data.frame(condition_KO_NT_vs_WT_NT) %>% 
...
# Write tables:
write.table(annot_final,file = "condition_KO_NT_vs_WT_NT.xls", sep = "\t")
...
```{r}
 # O alternativamente solo dplyr con library(dplyr)
library(tidyverse) 

# Crear una nueva columna para el -log10 de los valores FDR
annot_final <- annot_final %>%
    mutate(log10FDR = -log10(FDR))

# Crear el volcano plot con las personalizaciones solicitadas
volcano_plot <- ggplot(annot_final, aes(x = logFC, y = log10FDR)) +
    # Colorear genes downregulados en azul y upregulados en rojo, otros en gris
    ...
    # Líneas de corte en gris
    geom_hline(yintercept = -log10(0.05), linetype = "dashed", color = "grey") +
    geom_vline(xintercept = c(-1, 1), linetype = "dashed", color = "grey") +
    # Ajustar los ejes para ser líneas sólidas
    theme_minimal(base_family = "Arial") +  # Usar Arial para toda la fuente
    theme(axis.line = element_line(color = "black"),  # Líneas de los ejes sólidas y negras
          panel.grid.minor = element_blank(),  # Sin líneas de grilla menores
          panel.grid.major = element_blank(),  # Sin líneas de grilla mayores
          legend.position = "top",             # Mover la leyenda al tope
...
# Guardar el volcano plot con dimensiones iguales para ancho y alto
ggsave("volcano_plot_square.png", plot = volcano_plot, width = 6, height = 6, dpi = 300)
